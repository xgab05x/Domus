...
import { Link2, LayoutGrid, Pencil, Palette } from "lucide-react";
...
export default function GroupBubble({ group, onEdit }) {
  ...
  if (group.kind === "panel") {
    return (
      <div className="glass rounded-[28px] p-4 bubble sm:col-span-2" data-testid={`group-bubble-${group.id}`}>
        <div className="flex items-start gap-3">
          <div className="icon-btn icon-on w-11 h-11 shrink-0"><Icon size={19} /></div>
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-sm truncate">{group.name}</div>
            <div className="label mt-0.5">Placca · {members.length} tasti</div>
          </div>
        ...
        <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2">
          {members.map((m) => {
            ...
            return (
              <div key={m.id} className={`relative rounded-2xl transition-colors ${on ? "icon-on" : "glass-inner"}`}>
                <button onClick={() => updateEntity(m.id, { state: { on: !on } })} data-testid={`panel-btn-${group.id}-${m.id}`} className="w-full text-left p-3">
                  ...
                  <div className="text-[10px] opacity-70 mt-0.5">{on ? "acceso" : "spento"}</div>
                </button>
                {editable && (
  ...
  return (
    <div className="glass rounded-[28px] p-4 bubble" data-testid={`group-bubble-${group.id}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-semibold text-sm truncate">{group.name}</div>
          <div className="label mt-0.5 flex items-center gap-1"><Link2 size={10} /> {members.length} dispositivi collegati</div>
        </div>
        <button onClick={onEdit} className="btn-ghost w-7 h-7 rounded-full flex items-center justify-center text-muted" data-testid={`group-edit-${group.id}`} aria-label="Modifica gruppo"><Pencil size={13} /></button>
      </div>
